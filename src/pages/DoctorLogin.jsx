...
export default function DoctorLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const doctor = allDoctors.find(d => d.email === credentials.email);
      ...
      if (!doctor.profile_completed) {
        navigate(createPageUrl("DoctorProfile") + `?id=${doctor.id}`);
      } else {
        navigate(createPageUrl("DoctorDashboard") + `?id=${doctor.id}`);
      }
    } catch (error) {
